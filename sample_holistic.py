...
def main():
    ...
    prueba.writelines('{ind},{cordx},{cordy},{hand},{frame}'.format(ind='index', cordx='CY', cordy='CX', hand='hand',frame='frame'))
    prueba.writelines(',{hand1}'.format(hand1='hand1'))
    prueba.writelines('\n')
    ...
    while cap.isOpened():
        ...
        if left_hand_landmarks is not None:
            ...
            # 描画
            debug_image = draw_hands_landmarks(debug_image, cx, cy, left_hand_landmarks, upper_body_only, prueba, frame_i, 'R')
            debug_image = draw_bounding_rect(use_brect, debug_image, brect)
        ...
        if right_hand_landmarks is not None:
            ...
            # 描画
            debug_image = draw_hands_landmarks(debug_image, cx, cy, right_hand_landmarks, upper_body_only, prueba, frame_i,'L')
            debug_image = draw_bounding_rect(use_brect, debug_image, brect)
...
def draw_hands_landmarks(image, cx, cy, landmarks, upper_body_only, prueba, frame_i, handedness_str='R'):
    image_width, image_height = image.shape[1], image.shape[0]
    ...
    for index, landmark in enumerate(landmarks.landmark):
        ...
        prueba.writelines('{ind}.{cordx},{cordy},{hand},{frame}\n'.format(ind=index, cordx=landmark_x, cordy=landmark_y, hand=handedness_str,frame=frame_i))
