...
def main():
    ...
    # カメラ準備 ###############################################################
    cap = cv.VideoCapture('MonaChinaDeZaid.mp4')
    ...
    #prueba_1.writelines('{tim},{frame},'.format(tim='Time',frame='frame'))
    
    #prueba_1.writelines('{Name},{cordx},{cordy},'.format(Name='Name', cordx='CY', cordy='CX'))
...
def draw_hands_landmarks(image, cx, cy, landmarks, upper_body_only,prueba,prueba_3,tiemp, frame_i,handedness_str='R'):
    ...
    prueba.writelines('\n')
    prueba_3.writelines('\n')
    prueba.writelines('{Time},{frame},'.format(Time=tiemp,frame=frame_i))
    prueba_3.writelines('{Time},{frame},'.format(Time=tiemp,frame=frame_i))
    # キーポイント
    for index, landmark in enumerate(landmarks.landmark):
        if landmark.visibility < 0 or landmark.presence < 0:
            prueba_3.writelines('{Nam},{cordx},{cordy},'.format(Nam='Mano_'+str(index)+'_'+str(handedness_str), cordx='a', cordy='a'))
            continue
        ...
        prueba.writelines('{Nam},{cordx},{cordy},'.format(Nam='Mano_'+str(index)+'_'+str(handedness_str), cordx=landmark_x, cordy=landmark_y))
        prueba_3.writelines('{Nam},{cordx},{cordy},'.format(Nam='Mano_'+str(index)+'_'+str(handedness_str), cordx=landmark_x, cordy=landmark_y))
...
def draw_face_landmarks(image, landmarks, prueba,prueba_1,tiemp,frame_i):
    ...
    prueba.writelines('\n')
    prueba.writelines('{Time},{frame},'.format(Time=tiemp,frame=frame_i))
    prueba_1.writelines('{Time},{frame},'.format(Time=tiemp,frame=frame_i))

    for index, landmark in enumerate(landmarks.landmark):
        if landmark.visibility < 0 or landmark.presence < 0:
            #prueba_1.writelines('{nam},{cordx},{cordy},'.format(nam='face_'+str(index), cordx='Zaid', cordy='Zaid'))
            continue
        ...
        prueba.writelines('{nam},{cordx},{cordy},'.format(nam='Face'+str(index), cordx=landmark_x, cordy=landmark_y))
        prueba_1.writelines('{nam},{cordx},{cordy},'.format(nam='face_'+str(index), cordx=landmark_x, cordy=landmark_y,))
    prueba_1.writelines('\n')
...
def draw_pose_landmarks(image, landmarks, upper_body_only, prueba, prueba_2, tiemp,frame_i, visibility_th=0.5):
    ...
    prueba.writelines('\n')
    prueba.writelines('{Time},{frame},'.format(Time=tiemp,frame=frame_i,))
    prueba_2.writelines('{Time},{frame}'.format(Time=tiemp,frame=frame_i,))
    for index, landmark in enumerate(landmarks.landmark):
        landmark_x = min(int(landmark.x * image_width), image_width - 1)
        landmark_y = min(int(landmark.y * image_height), image_height - 1)
        landmark_z = landmark.z
        ...
        if landmark.visibility < visibility_th:
            prueba_2.writelines(',{Nam},{cordx},{cordy}'.format(Nam='body'+ str(index), cordx='a', cordy='a'))
            continue
        ...
        prueba.writelines('{Nam},{cordx},{cordy},'.format(Nam='Inferior_'+str(index), cordx=landmark_x, cordy=landmark_y))            
        prueba_2.writelines(',{Nam},{cordx},{cordy}'.format(Nam='body'+ str(index), cordx=landmark_x, cordy=landmark_y))            
    prueba_2.writelines('\n')
    if len(landmark_point) > 0:
        # 右目
